...
export async function activate(context: vscode.ExtensionContext) {
	...
	const openClineInNewTab = async () => {
		...
		const panel = vscode.window.createWebviewPanel(VscodeWebviewProvider.TAB_PANEL_ID, "Kline", targetCol, {
			enableScripts: true,
	...
	*/
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("cline.openWalkthrough", async () => {
			await vscode.commands.executeCommand("workbench.action.openWalkthrough", "kd-saoudrizwan.kd-cline#ClineWalkthrough")
...
function setupHostProvider(context: ExtensionContext) {
	...
	const getCallbackUri = async () => `${vscode.env.uriScheme || "vscode"}://kd-saoudrizwan.kd-cline`
