...
`,
			...
			if (err.message.includes("ENAMETOOLONG")) {
				throw new Error(
					`Executing Claude Code failed due to a long system prompt. Windows has a limit of 8191 characters, which makes the integration with Kline not work properly.
Please check our docs on how to integrate Claude Code with Kline on Windows: https://docs.cline.bot/provider-config/claude-code#windows-setup.
Anthropic is aware of this issue and is considering a fix: https://github.com/anthropics/claude-code/issues/3411.
`,
					{ cause: err },
				)
...
function runProcess(
	...
	const args = [
		...
		"--disallowedTools",
		claudeCodeTools,
		// Kline will handle recursive calls
		"--max-turns",
		"1",
		"--model",
		modelId,
