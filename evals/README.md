# Kline Evaluation System

This directory contains the evaluation system for benchmarking Kline against various coding evaluation frameworks.

## Overview

The Kline Evaluation System allows you to:

1. Run Kline against standardized coding benchmarks
...
- Node.js 16+
- VSCode with Kline extension installed
- Git

### Activation Mechanism

The evaluation system uses an `evals.env` file approach to activate test mode in the Kline extension. When an evaluation is run:

1. The CLI creates an `evals.env` file in the workspace directory
2. The Kline extension activates due to the `workspaceContains:evals.env` activation event
...
The Kline Evaluation System includes a specialized suite for evaluating how well models can make precise edits to files using the `replace_in_file` tool.
