...
When your conversation approaches the model's context window limit, Kline automatically summarizes it to free up space and keep working. 
...
Kline monitors token usage during your conversation. When you're getting close to the limit, he:
...
Previously, Kline would truncate older messages when hitting context limits. This meant losing important context from earlier in the conversation.
...
1. Kline uses a [summarization prompt](https://github.com/cline/cline/blob/main/src/core/prompts/contextManagement.ts) to request a summary of the conversation.

2. Once the summary is generated, Kline replaces the conversation history with a [continuation prompt](https://github.com/cline/cline/blob/main/src/core/prompts/contextManagement.ts#L69) that asks Kline to keep working and provides the summary as context.
...
<Note>
When using other models, Kline automatically falls back to the standard rule-based context truncation method, even if Auto Compact is enabled in settings.
</Note>
