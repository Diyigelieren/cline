...
When you interact with code in your editor, you can access Kline commands in two ways:
...
The "Add to Cline" command sends your selected code to the Kline chat panel. This is perfect for:
...
When you use this command, Kline automatically includes:
...
When you see a lightbulb icon in your editor, click it to access these Kline commands:
...
3. Sends both to Kline with a request to fix the issues
...
4. View the result in the Kline chat panel
...
4. Choose the appropriate Kline command (Fix, Explain, or Improve)
5. View the result in the Kline chat panel
...
2. **Metadata Collection**: Kline gathers important metadata:
...
4. **Integration with Chat**: The results appear in the Kline chat panel, where you can:
...
-   **Select complete logical units**: When possible, select entire functions, classes, or modules to give Kline complete context
-   **Include imports**: For language-specific help, include relevant imports so Kline understands dependencies
