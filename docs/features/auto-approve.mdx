The Auto Approve menu lets you set fine-grained permissions on what you allow Kline to do in an automated way.
...
By default, Kline will ask for your permission before calling any tool, including reading or writing files.

If you want to allow Kline to do something without asking, you can set the Auto Approve permission for that tool.

## Permission Options

-   **Read project files**

    -   Allows Kline to read files within your current workspace without asking
    -   **Read all files**
        -   Extends read permission to files outside your workspace (system files, config files, etc.)

-   **Edit project files**

    -   Allows Kline to modify files within your current workspace without confirmation
    -   **Edit all files**
...
-   **Use the browser**
    ...
    -   Allows Kline to use the browser tool to fetch web content
...
-   **Maximum requests**
    -   Sets the number of consecutive automated actions Kline can take before requiring your input
...
This gives Kline enough freedom to explore your codebase without constant interruptions, while still requiring permission for edits or potentially destructive actions.
