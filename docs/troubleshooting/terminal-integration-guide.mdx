...
This guide helps you resolve terminal integration issues in Cline. Terminal integration is crucial for Kline to execute commands and read their output, enabling it to understand errors, test results, and command responses.

<Tip>
	If you're experiencing terminal issues, try switching to a simpler shell like `bash` in the Kline settings, under "Terminal Settings"
...
```mermaid
graph TD
    A[Terminal Issue] --> B{Can Kline execute commands?}
    B -->|No| C[Shell Integration Unavailable]
    B -->|Yes| D{Can Kline see the output?}
...
-   Commands execute but Kline can't read output
...
-   Kline states in chat: "[Command is running but producing no output]"
-   Commands complete but Kline doesn't see results
...
-   Recommend AI to use file output instead
    1. Tell Kline in chat or Kline rules, to use `command > output.txt` before reading the file/s

<Tip>
	This family of issues is only partially solved in the latest Kline versions, so if you still face this, create a GitHub issue
...
-   The Kline button "Proceed while running" doesn't work properly in chat

<Tip>
	This family of issues has been solved in latest Kline versions but if you still face any issues, then create a GitHub issue
	for this.
</Tip>

## Terminal Settings Explained

Access these in Kline by clicking the settings icon, and navigating to the "Terminal Settings" section:

### Default Terminal Profile

-   **What it does**: Selects which shell Kline uses for commands
...
-   **What it does**: How long Kline waits for the terminal to be ready
-   **Default**: 4 seconds
-   **When to increase**:
...
-   **What it does**: Limits how many lines Kline reads from terminal output
-   **Default**: 500 lines
-   **When to adjust**:
...
1. Install Kline on the remote machine, not locally
...
1. Install Kline in the container
...
### Why does Kline create so many terminals?

When shell integration fails, Kline can't reuse terminals safely (they might be running long processes). Enable shell integration or adjust the terminal reuse setting.

### Can I use my custom shell (nushell, xonsh, etc.)?

Kline officially supports bash, zsh, fish, and PowerShell. Custom shells may work but aren't guaranteed. Use bash as a fallback.
...
2. **Report the Issue**:
    - Use `/reportbug` in Kline github issues
