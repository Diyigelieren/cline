---
title: "Installing Kline for JetBrains (Early Access)"
description: "Get early access to Kline in your favorite JetBrains IDE with the same powerful AI assistance you know from VSCode."
---
...
Kline for JetBrains works almost identically to Kline in VSCode. All the core features work properly: diff editing, using tools, logging in with different providers, MCP servers, Kline rules and workflows, and more.

<Frame>
	<img
		src="https://storage.googleapis.com/cline_public_images/docs/assets/jetbrains-demo-hifi.gif"
		alt="Kline running in JetBrains IDE showing AI assistance"
	/>
</Frame>

<Note>Kline for JetBrains is in early access. All core features are functional, with ongoing improvements based on user feedback.</Note>

## Installation

As part of our early access program, Kline for JetBrains is available through direct download before its official marketplace release. You'll need to install it manually from a downloaded file:
...
<Frame>
	<img
		...
		alt="JetBrains plugin marketplace showing Kline download page"
	/>
</Frame>
...
<Frame>
	<img
		...
		alt="File selection dialog showing Kline plugin zip file"
	/>
</Frame>
...
After installation, you'll find Kline in your IDE:

1. **Open Cline:**
   - Look for the Kline tool window (usually on the right side)
   - Or go to **View** → **Tool Windows** → **Cline**

2. **Sign In (optional, BYOK is also available):**
   - Click **Sign In** in the Kline panel
...
While Kline for JetBrains includes all the same powerful features, there's one important difference to be aware of:

**Terminal Integration:** The terminal inside JetBrains isn't integrated with Kline the same way it is in VSCode. Kline can execute commands, but the output will only appear in the webview if you expand the **Command Output** section.
...
- **Diff Editing:** Kline can read, write, and edit files with the same precision
...
- **Kline Rules:** Custom instructions and workflows work the same way
...
- **Project Context:** Kline automatically understands your project structure, just like in VSCode
- **Language Support:** Kline works with any language your JetBrains IDE supports
- **Debugging Help:** Share error messages and stack traces directly in the chat
- **Code Review:** Ask Kline to review your code changes before committing

## Troubleshooting

If you don't see the Kline tool window after installation:
...
Now that you have Kline installed, you might want to:
...
- Set up [Kline rules](/features/cline-rules) for your specific workflow
