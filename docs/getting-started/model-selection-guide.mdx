...
New models drop constantly, so this guide focuses on what's working well with Kline right now. We'll keep it updated as the landscape shifts.
...
Check [OpenRouter's Kline usage stats](https://openrouter.ai/apps?url=https%3A%2F%2Fcline.bot%2F) to see real usage patterns from the community.

## Context Management

Kline automatically handles context limits with [auto-compact](/features/auto-compact). When you approach your model's limit, Kline summarizes the conversation to keep working. You don't need to micromanage this.
