...
Utilizing MCP servers will increase your token usage. Kline offers the ability to restrict or disable MCP server functionality as desired.

1. Click the "MCP Servers" icon in the top navigation bar of the Kline extension.
2. Select the "Installed" tab, and then Click the "Advanced MCP Settings" link at the bottom of that pane.
3. Kline will open a new settings window. find `Cline>Mcp:Mode` and make your selection from the dropdown menu.
...
1. Click the MCP Servers icon at the top navigation bar of the Kline pane.
...
_Example of MCP Server config in Kline (STDIO Transport)_
...
After configuring an MCP server, Kline will automatically detect available tools and resources. To use them:

1. Type your request in Cline's conversation window
2. Kline will identify when an MCP tool can help with your task
