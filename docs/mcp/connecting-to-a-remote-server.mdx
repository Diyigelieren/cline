...
description: "The Model Context Protocol (MCP) allows Kline to communicate with external servers that provide additional tools and resources to extend its capabilities. This guide explains how to add and connect to remote MCP servers through the MCP Servers interface."
---
...
1. Click on the Kline icon in the VSCode sidebar
2. Open the menu (⋮) in the top right corner of the Kline panel
...
4. Kline will attempt to connect to the server and display the connection status
...
> **Warning**: The following third-party marketplaces are listed for informational purposes only. Kline does not endorse, verify, or take responsibility for any servers listed on these marketplaces. These servers are cloud-hosted services that process your requests and may have access to data you share with them. Always review privacy policies and terms of use before connecting to third-party services.
...
2. **Request Timeout**:
    - Set how long Kline should wait for server responses
...
-   **Enabled**: Kline can use the server's tools and resources
...
For advanced users, Kline stores MCP server configurations in a JSON file that can be modified:
...
Once connected, Kline can use the tools and resources provided by the MCP server. When Kline suggests using an MCP tool:
