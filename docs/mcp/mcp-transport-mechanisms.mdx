...
description: "Learn about the two primary transport mechanisms for communication between Kline and MCP servers: Standard Input/Output (STDIO) and Server-Sent Events (SSE). Each has distinct characteristics, advantages, and use cases."
---

Model Context Protocol (MCP) supports two primary transport mechanisms for communication between Kline and MCP servers: Standard Input/Output (STDIO) and Server-Sent Events (SSE). Each has distinct characteristics, advantages, and use cases.
...
-   Single-client scenarios (one Kline instance per server)
...
-   **Remote Access**: Can be hosted on a different machine from your Kline instance
...
-   **Execution**: Starts and stops with Kline (child process lifecycle)
...
-   Need to be installed alongside Kline or via a package manager
