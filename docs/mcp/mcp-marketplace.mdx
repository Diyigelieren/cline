...
MCP servers are specialized extensions that enhance Cline's capabilities. They enable Kline to perform additional tasks like fetching web pages, processing images, accessing APIs, and much more.
...
-   If the server requires an API key (most do), Kline will guide you through:
...
-   Kline will show confirmation when installation is complete
-   Check the server status in Cline's MCP settings UI

### 5. Using Your New Server

-   After successful installation, Kline will automatically integrate the server's capabilities
-   You'll see new tools and resources available in Cline's system prompt
-   Simply ask Kline to use the capabilities of your new server
-   Example: "Search the web for recent React updates using Perplexity"

**Corporate Users:** If you're using Kline in a corporate environment, ensure you have permission to install third-party MCP servers according to your organization's security policies.
...
-   This file is located at: `/Users/<username>/Library/Application Support/Code/User/globalStorage/kd-saoudrizwan.kd-cline/settings/cline_mcp_settings.json`
-   Environment variables (like API keys) are securely stored
-   The server path is registered

### 3. Server Launch

-   Kline detects the configuration change
-   Kline launches your server as a separate process
...
-   Kline can now use these capabilities when prompted by the user
...
1. Open the MCP settings file: `/Users/<username>/Library/Application Support/Code/User/globalStorage/kd-saoudrizwan.kd-cline/settings/cline_mcp_settings.json`
...
-   Use a terminal and run the command with its arguments directly. This will allow you to see the same errors that Kline is seeing

## MCP Server Rules

Kline is already aware of your active MCP servers and what they are for, but when you have a lot of MCP servers enabled, it can be useful to define when to use each server.
...
-   **Contextual Tool Selection**: Kline selects appropriate tools based on conversation context
...
When you write "Can you scrape this website?", Kline detects "scrape" and "website" as triggers, automatically selecting web-related MCP servers.
