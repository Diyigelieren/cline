---
title: "MCP Overview"
description: "Learn about Model Context Protocol (MCP) servers, their capabilities, and how Kline can help build and use them. MCP standardizes how applications provide context to LLMs, acting like a USB-C port for AI applications."
---
...
Kline does not come with any pre-installed MCP servers. You'll need to find and install them separately.
...
-   **Kline Marketplace:** Install one from Cline's [MCP Marketplace](/mcp/mcp-marketplace)
-   **Ask Cline:** You can ask Kline to help you find or create MCP servers
...
Kline simplifies the building and use of MCP servers through its AI capabilities.

### Building MCP Servers

-   **Natural language understanding:** Instruct Kline in natural language to build an MCP server by describing its functionalities. Kline will interpret your instructions and generate the necessary code.
-   **Cloning and building servers:** Kline can clone existing MCP server repositories from GitHub and build them automatically.
-   **Configuration and dependency management:** Kline handles configuration files, environment variables, and dependencies.
-   **Troubleshooting and debugging:** Kline helps identify and resolve errors during development.

### Using MCP Servers

-   **Tool execution:** Kline seamlessly integrates with MCP servers, allowing you to execute their defined tools.
-   **Context-aware interactions:** Kline can intelligently suggest using relevant tools based on conversation context.
-   **Dynamic integrations:** Combine multiple MCP server capabilities for complex tasks. For example, Kline could use a GitHub server to get data and a Notion server to create a formatted report.
