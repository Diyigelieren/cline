...
> 🚀 **Build and share your MCP servers with the world.** Once you've created a great MCP server, submit it to the [Kline MCP Marketplace](https://github.com/cline/mcp-marketplace) to make it discoverable and one-click installable by thousands of developers.

## What Are MCP Servers?

Model Context Protocol (MCP) servers extend AI assistants like Kline by giving them the ability to:
...
A `.clinerules` file is a special configuration that Kline reads automatically when working in the directory where it's placed. These files:

-   Configure Cline's behavior and enforce best practices
-   Switch Kline into a specialized MCP development mode
...
When this `.clinerules` file is present in your working directory, Kline will:
...
First, add a `.clinerules` file to the root of your MCP working directory using the protocol above. This file configures Kline to use the MCP development protocol when working in this folder.

### 2. Start a Chat with a Clear Description

Begin your Kline chat by clearly describing what you want to build. Be specific about:
...
Kline will automatically start in PLAN MODE, guiding you through the planning process:
...
One of the most effective ways to help Kline build your MCP server is to share official API documentation right at the start:
...
In this collaborative phase, you work with Kline to design your MCP server:
...
Once planning is complete, Kline helps implement the server:
...
Resources let your MCP servers expose data to Kline without executing code. They're perfect for providing context like files, API responses, or database records that Kline can reference during conversations.
...
Resources make your MCP servers more context-aware, allowing Kline to access specific information without requiring you to copy/paste. For more information, refer to the [official documentation](https://modelcontextprotocol.io/docs/concepts/resources).
