...
1. **The `new_task` Tool:**
    ...
    - **Context Preloading:** Crucially, Kline can **preload** this new task session with specific context provided within the tool's `<context>` block. This context can be anything Kline or a `.clinerules` file defines – summaries, code snippets, next steps, project state, etc.
2. **Context Window Awareness:**
    - **Tracking:** Kline internally tracks the percentage of its available context window currently being used during a task.
    - **Visibility:** This information is visible in the `environment_details` provided to Kline in its prompt.

#### Using the `/newtask` Slash Command

As a quick alternative to Kline suggesting the `newtask` tool or defining complex rules, you can directly initiate the process using a Slash Command.

-   **How:** Simply type `/newtask` in the chat input field.
-   **Action:** Kline will propose creating a new task, typically suggesting context based on the current session (similar to its default behavior when using the tool). You will still get the `ask_followup_question` prompt to confirm and potentially modify the context before the new task is created.
-   **Benefit:** Provides a fast, user-initiated way to leverage the `new_task` functionality for branching explorations or managing long sessions without waiting for Kline to suggest it.
...
-   **Tool Availability:** The `new_task` tool exists, and Kline _can_ choose to use it.
-   **Context Awareness:** Kline _is_ aware of its context usage percentage.
-   **No Automatic Trigger:** Kline **will not** automatically initiate a task handoff _solely_ based on context usage reaching a specific percentage (like 50%). The decision to suggest using `new_task` comes from the AI model's reasoning based on the overall task progress and prompt instructions.
-   **Basic Context Preloading:** If `new_task` is used without specific rules defining the `<context>` block structure, Kline will attempt to preload relevant information based on its current understanding (e.g., a basic summary of progress and next steps), but this may be less comprehensive than a rule-driven approach.

#### The Power of `.clinerules`: Enabling Custom Workflows

While the core capabilities exist by default, the true power, automation, and customization emerge when you combine `new_task` and context awareness with custom workflows defined in `.clinerules`. This allows you to precisely control _when_ and _how_ Kline manages context and task continuity.
...
-   **Intelligent Breakpoints:** Instruct Kline via rules to find logical stopping points (e.g., after completing a function or test) _after_ a context threshold is passed, ensuring cleaner handoffs.
-   **Structured Task Decomposition:** Use Plan Mode to define subtasks, then use `.clinerules` to have Kline automatically create a new task via `new_task` upon completing each subtask, preloading the context for the _next_ subtask.
...
1. **Trigger Identification (Rule-Based):** Kline monitors for handoff points defined in the rules (e.g., context usage > 50%, task completion).
2. **User Confirmation:** Kline uses `ask_followup_question` to propose creating a new task, often showing the intended context defined by the rules.
...
3. **User Control:** You can approve, deny, or ask Kline to modify the context before the new task is created.
4. **Context Packaging (`new_task` Tool):** If approved, Kline uses `new_task`, packaging the context according to the structure mandated by the `.clinerules`.
...
-   **Intelligent Breakpoints:** Instruct Kline to find logical stopping points (e.g., after completing a function or test) _after_ a context threshold is passed, ensuring cleaner handoffs.
-   **Structured Task Decomposition:** Use Plan Mode to define subtasks, then use `.clinerules` to have Kline automatically create a new task via `new_task` upon completing each subtask.
...
-   **User Approval:** You always have control, approving the handoff and having the chance to modify the context Kline proposes to carry forward.
