...
description: "Remote browser support allows Kline to utilize a remote Chrome instance, leveraging authentication tokens and session cookies relevant to certain web development test cases."
icon: globe-pointer
---

The Remote Browser feature in Kline allows the AI assistant to interact with web content directly through a controlled browser instance. This enables several powerful capabilities:
...
Remote Browser allows Kline to view and interact with websites directly. This feature enables Kline to:
...
You can ask Kline to use the browser with simple instructions:
...
Kline can only use one browser at a time. If you want to visit a different website, you can either:

-   Ask Kline to navigate to a new URL within the same browser session
-   Ask Kline to close the current browser and open a new one

#### Browser Must Be Closed Before Using Other Tools

If you want Kline to edit files or run commands after using the browser, you must first ask it to close the browser:
...
The browser has a fixed viewport size (900x600 pixels by default), similar to a small laptop screen. Kline will share screenshots after each action so you can see exactly what it sees.

#### Console Logs

Kline captures browser console logs, which can be helpful for debugging web applications. These logs are included with each screenshot.

### Common Use Cases

-   **Web Development**: Test your websites and web applications
-   **UI/UX Review**: Get feedback on website design and usability
-   **Content Research**: Have Kline browse websites to gather information
...
-   **If the browser seems stuck**: Ask Kline to close the browser and try again
...
Kline should now be able to use the Remote Browser feature from within WSL.
