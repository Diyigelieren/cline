...
Kline supports accessing models directly through the official OpenAI API.
...
3.  **Create a Key:** Click on "Create new secret key". It's good practice to give your key a descriptive name (e.g., "Kline API Key").
...
Kline is compatible with a variety of OpenAI models, including but not limited to:
...
1.  **Open Kline Settings:** Click the settings gear icon (⚙️) in the Kline panel.
...
-   **Azure OpenAI Service:** If you are looking to use the Azure OpenAI service, please note that specific documentation for Azure OpenAI with Kline may be found separately, or you might need to configure it as an OpenAI-compatible endpoint if such functionality is supported by Kline for custom configurations.
